...
const upload = (0, multer_1.default)({ storage: multer_1.default.memoryStorage(), limits: { fileSize: 10 * 1024 * 1024 } }); // 10MB max
...
async function getParentChildren(profileId) {
    ...
    const studentIds = links.map((link) => link.student_id).join(',');
...
async function getStudentClasses(studentId) {
    const SUPABASE_URL = 'https://wlgclriinxtyctaadiql.supabase.co';
    const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndsZ2Nscmlpbnh0eWN0YWFkaXFsIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MjAzNzA2NywiZXhwIjoyMDg3NjEzMDY3fQ.Nkny8TqAH40_E8KoVQbBgtVg7L3fWnmP0eB208iLmp4';
    const res = await fetch(`${SUPABASE_URL}/rest/v1/students?id=eq.${studentId}&select=id,class_id,classes:class_id(id,name,academic_year_id)`, { headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` } });
    const data = await res.json();
    if (!res.ok)
        throw new error_middleware_1.AppError('Failed to fetch student classes', 500);
    return (data || [])
        .filter((s) => s.class_id)
        .map((s) => ({
        class_id: s.class_id,
...
router.get('/teachers', async (req, res, next) => {
    try {
        ...
        const { data: teachers } = await supabase_1.supabaseAdmin
            .from('teachers')
            .select(`
        id,
        profile_id,
...
router.get('/children/:childId/classes', async (req, res, next) => {
    try {
        ...
        res.json((0, pagination_1.successResponse)(classes));
    }
    catch (err) {
        next(err);
    }
...
router.get('/children/:childId/schedule', async (req, res, next) => {
    try {
        ...
        const children = await getParentChildren(req.user.id);
        const hasChild = children.some((c) => c.student_id === childId);
        if (!hasChild)
            throw new error_middleware_1.AppError('Accès non autorisé à cet enfant', 403);
...
router.get('/grades', async (req, res, next) => {
    try {
        ...
        if (!childId)
            throw new error_middleware_1.AppError('childId est requis', 400);
        const children = await getParentChildren(req.user.id);
        const hasChild = children.some((c) => c.student_id === childId);
        if (!hasChild)
            throw new error_middleware_1.AppError('Accès non autorisé à cet enfant', 403);
...
router.post('/attendance/:attendanceId/justify', upload.single('justification_pdf'), async (req, res, next) => {
    try {
        ...
        if (!reason || !reason.trim())
            throw new error_middleware_1.AppError('La raison de justification est requise', 400);
        // Vérifier que le fichier est bien un PDF si fourni
        if (req.file && req.file.mimetype !== 'application/pdf') {
            throw new error_middleware_1.AppError('Seuls les fichiers PDF sont acceptés', 400);
        }
        // 1. Récupérer l'enregistrement d'absence
        const resAtt = await fetch(`${SUPABASE_URL}/rest/v1/attendance?id=eq.${attendanceId}&select=id,student_id,status`, { headers: H });
        const attArr = (await resAtt.json());
        if (!resAtt.ok || !attArr.length)
            throw new error_middleware_1.AppError('Absence introuvable', 404);
        const att = attArr[0];
        if (att.status !== 'absent' && att.status !== 'late') {
            throw new error_middleware_1.AppError('Seules les absences ou retards peuvent être justifiés', 400);
        }
        // 2. Vérifier accès parent
        const children = await getParentChildren(req.user.id);
        const hasChild = children.some((c) => c.student_id === att.student_id);
        if (!hasChild)
            throw new error_middleware_1.AppError('Accès non autorisé', 403);
        // 3. Upload PDF si fourni
        let justificationUrl = null;
        if (req.file) {
            justificationUrl = await (0, storage_1.uploadFile)(storage_1.STORAGE_BUCKETS.DOCUMENTS, req.file, `justifications/${att.student_id}`);
        }
        ...
        const resUpdate = await fetch(`${SUPABASE_URL}/rest/v1/attendance?id=eq.${attendanceId}`, { method: 'PATCH', headers: H, body: JSON.stringify(updatePayload) });
        const updated = (await resUpdate.json());
        if (!resUpdate.ok)
            throw new error_middleware_1.AppError('Échec de la mise à jour', 500);
        res.json((0, pagination_1.successResponse)(updated[0] || { id: attendanceId }, 'Absence justifiée avec succès'));
    }
    catch (err) {
        next(err);
    }
...
router.get('/assignments', async (req, res, next) => {
    try {
        ...
        const studentClasses = await getStudentClasses(childId);
        const classIds = studentClasses.map((sc) => sc.class_id).join(',');
        if (!classIds) {
            return res.json((0, pagination_1.successResponse)([]));
        }
...
router.get('/children/:childId/submissions', async (req, res, next) => {
    try {
        ...
        const subsRes = await fetch(`${SUPABASE_URL}/rest/v1/submissions?student_id=eq.${childId}&select=*&order=submitted_at.desc`, { headers: H });
        ...
        const commentsRes = await fetch(`${SUPABASE_URL}/rest/v1/teacher_comments?submission_id=in.(${submissionIds})&select=*&order=created_at.asc`, { headers: H });
        const comments = (await commentsRes.json());
        // 3. Enrichir chaque soumission avec ses commentaires
        const commentsMap = new Map();
        for (const c of (comments || [])) {
            if (!commentsMap.has(c.submission_id))
                commentsMap.set(c.submission_id, []);
            commentsMap.get(c.submission_id).push(c);
        }
        const enriched = submissions.map((sub) => {
            const subComments = commentsMap.get(sub.id) || [];
            const teacherComment = subComments.find((c) => c.comment_type === 'teacher_feedback');
            const studentReply = subComments.find((c) => c.comment_type === 'student_reply');
            return {
                ...sub,
        ...
        res.json((0, pagination_1.successResponse)(enriched));
    }
    catch (err) {
        next(err);
    }
...
router.get('/schedule', async (req, res, next) => {
    try {
        ...
        if (!targetClassId) {
            throw new error_middleware_1.AppError('classId ou childId est requis', 400);
        }
        const url = `${SUPABASE_URL}/rest/v1/schedule_slots?class_id=eq.${targetClassId}&is_active=eq.true&select=*,subjects:subject_id(name,color),teachers:teacher_id(profiles:profile_id(first_name,last_name))&order=day_of_week,start_time`;
        const resData = await fetch(url, { headers: H });
        const data = (await resData.json());
...
router.get('/stats', async (req, res, next) => {
    try {
        ...
        const assignmentsData = classIds ? (await assignmentsRes.json()) : [];
...
router.post('/messages', async (req, res, next) => {
    try {
        ...
        const dataArr = (await resInsert.json());
...
router.patch('/notifications/:id/read', async (req, res, next) => {
    try {
        ...
        const dataArr = (await resUpdate.json());
        const data = dataArr[0];
        if (!resUpdate.ok)
            throw new error_middleware_1.AppError('Failed to mark notification as read', 500);
        res.json((0, pagination_1.successResponse)(data, 'Notification marquée comme lue'));
    }
    catch (err) {
        next(err);
    }
...
router.get('/profile', async (req, res, next) => {
    try {
        ...
        res.json((0, pagination_1.successResponse)(profile));
    }
    catch (err) {
        next(err);
    }
...
router.get('/my-id', async (req, res, next) => {
    try {
        const { data: parent, error } = await supabase_1.supabaseAdmin
            .from('parents')
            .select('id')
        ...
        return res.json((0, pagination_1.successResponse)(parent));
    }
    catch (err) {
        return next(err);
    }
...
router.get('/student-parent/:studentId', async (req, res, next) => {
    try {
        ...
        const { data: link } = await supabase_1.supabaseAdmin
            ...
            .single();
        if (!link)
            throw new error_middleware_1.AppError('Aucun parent trouvé pour cet étudiant', 404);
        return res.json((0, pagination_1.successResponse)({ parent_id: link.parent_id }));
    }
    catch (err) {
        return next(err);
    }
...
router.get('/profile-by-id/:parentId', async (req, res, next) => {
    try {
        ...
        const { data: profile } = await supabase_1.supabaseAdmin
            ...
            .single();
        return res.json((0, pagination_1.successResponse)(profile));
    }
    catch (err) {
        return next(err);
    }
