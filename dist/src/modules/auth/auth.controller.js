...
class AuthController {
    ...
    async resetPasswordWithToken(req, res, next) {
        try {
            const body = auth_schema_1.resetPasswordWithTokenSchema.parse(req.body);
            const result = await auth_service_1.authService.resetPasswordWithToken(body.token, body.password);
            return res.json(result);
        }
        catch (err) {
            return next(err);
