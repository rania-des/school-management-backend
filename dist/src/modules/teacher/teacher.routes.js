...
router.post('/grades', async (req, res, next) => {
    try {
        ...
        const { data, error } = await supabase_1.supabaseAdmin
            ...
            .single();
        if (error)
            throw new error_middleware_1.AppError(`Failed to create grade: ${error.message}`, 500);
...
router.put('/grades/:gradeId', async (req, res, next) => {
    try {
        ...
        const { data, error } = await supabase_1.supabaseAdmin
            ...
            .single();
        if (error)
            throw new error_middleware_1.AppError('Failed to update grade', 500);
        if (!data)
...
router.post('/assignments', async (req, res, next) => {
    try {
        ...
        if (!classId || !subjectId || !title || !dueDate) {
            throw new error_middleware_1.AppError('classId, subjectId, title et dueDate sont requis', 400);
        }
        const { data, error } = await supabase_1.supabaseAdmin
            .from('assignments')
            .insert({
            ...
            .single();
        if (error)
            throw new error_middleware_1.AppError(`Failed to create assignment: ${error.message}`, 500);
        ...
        res.status(201).json((0, pagination_1.successResponse)(data, 'Devoir créé avec succès'));
    }
    catch (err) {
        next(err);
    }
...
router.patch('/submissions/:submissionId/grade', async (req, res, next) => {
    try {
        ...
        const { data, error } = await supabase_1.supabaseAdmin
            ...
            .single();
        if (error)
            throw new error_middleware_1.AppError('Failed to grade submission', 500);
    ...
    try {
        ...
        for (const absent of absents) {
            const { data: student } = await supabase_1.supabaseAdmin
                ...
                .single();
            if (student) {
                const { data: parentLinks } = await supabase_1.supabaseAdmin
                    .from('parent_student')
...
router.post('/messages', async (req, res, next) => {
    try {
        ...
        const { data, error } = await supabase_1.supabaseAdmin
            ...
            .single();
        if (error)
            throw new error_middleware_1.AppError(`Failed to send message: ${error.message}`, 500);
